...
const ProductCard = ({ product }) => {
  ...
  return (
    <div className='bg-white rounded-2xl shadow-md overflow-hidden flex flex-col hover:shadow-lg transition-shadow duration-300'>
      <Link to={`/product/${product.id}`} className='flex justify-center items-center p-4'>
      ...
      <div className='px-4 py-2 flex flex-col flex-grow'>
        ...
        <button className='mt-auto bg-blue-600 text-white text-sm font-semibold py-2 px-4 rounded hover:bg-blue-700 transition-colors cursor-pointer' onClick={() => addToCart(product)}>Adicionar ao carrinho</button>
      </div>
    </div>
  )
}
